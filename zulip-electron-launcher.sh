...
envSetup()
{
	...
	cd $workingDir
	git remote -v | grep "github\.com.nottheswimmer.zulip-desktop.git (fetch)" > /dev/null 2>&1
	if [ $? -eq 0 ]
	then
		upstreamRemote=`git remote -v | grep "github\.com.nottheswimmer.zulip-desktop.git (fetch)" | awk '{ print $1 }'`
	else
